...
export { hasMeaningfulHypothesisValue } from "@/lib/onboarding/templates/icp-definition/dimension-renderers";
...
async function completionCheck(
  ...
  const profileRubric = profileRes.data?.icp_rubric
    ? safeParseIcpRubric(profileRes.data.icp_rubric)
    : null;
  if (profileRubric?.success) completedSteps.push(2);
...
async function normalizeScoringProfile(
  ...
  const icpRubric = coerceIcpRubric({
