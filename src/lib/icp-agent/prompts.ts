...
export function buildRevisionCriticPrompt({
  ...
  return [
    ...
    "V1 supports append-only patches to these string-array paths only:",
    ...
    "- /failed_workarounds",
    "- /aha",
    "Use target=rubric for /proof_points/* or /signals/* paths.",
    "Use target=narrative for /decision_criteria, /failed_workarounds, or /aha.",
    "Decline if the evidence requires replacing existing values, changing scalar fields, or making a broad rewrite.",
    "",
    "<current_icp_context>",
    renderIcpContextForPrompt(context),
    "</current_icp_context>",
...
export function buildJudgePrompt({
  ...
  return [
    ...
    "Approve only if every patch is append-only, evidence-backed, narrow, and useful.",
