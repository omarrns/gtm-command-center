...
import {
  getCommandItemsForUserType,
  type ModeCommandItem,
} from "@/lib/platform/modes/registry";
...
export function CommandPalette({ userType }: CommandPaletteProps) {
  ...
  const items = useMemo(() => getCommandItemsForUserType(userType), [userType]);
  ...
  function select(item: ModeCommandItem) {
    setOpen(false);
