...
async function assertIcpConfirmedState(userId: string, interviewId: string) {
  ...
  if (sp?.icp_rubric) {
    ...
    assert(
      typeof rubric.disqualifiers === "object" &&
        rubric.disqualifiers !== null &&
        Array.isArray(
          (rubric.disqualifiers as Record<string, unknown>)
            .behavioral_disqualifiers,
        ),
      "icp_rubric.disqualifiers is structured",
    );
    assert(
