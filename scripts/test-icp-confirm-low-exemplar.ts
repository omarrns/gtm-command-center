...
function unitTestAdapterShapeCoercion() {
  ...
  assert(
    good.edits.icp.firmographics.employee_range.min === 20,
    "well-shaped firmographics: employee_range.min parsed",
  );
  assert(
    good.edits.icp.firmographics.employee_range.max === 100,
    "well-shaped firmographics: employee_range.max parsed",
  );
  ...
  assert(
    tupleResult.edits.icp.firmographics.employee_range.min === 0,
    "malformed tuple firmographics: falls back to employee_range.min=0",
  );
  assert(
    tupleResult.edits.icp.firmographics.employee_range.max === 10000,
    "malformed tuple firmographics: falls back to employee_range.max=10000",
  );
...
async function integrationTestLowExemplar(userId: string) {
  ...
  if (sp?.icp_rubric) {
    ...
    const disqualifiers = rubric.disqualifiers as
      | { behavioral_disqualifiers?: unknown }
      | undefined;
    assert(
      Array.isArray(disqualifiers?.behavioral_disqualifiers) &&
        disqualifiers.behavioral_disqualifiers.includes("Enterprise-only"),
      "icp_rubric persists structured disqualifiers",
    );
  }
